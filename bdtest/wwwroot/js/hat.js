$( document ).ready(function() {
        $("#hat,#miniHat").css({"grid-template-columns":($(window).width() - $('#eat,#main,#main-finish').width())/2 + "px "+$('#eat,#main,#main-finish').width()/3+"px "+
        ...
        if($('*').is('#kinds-eat'))
        {
            ...
            $('#minihat-word').css({"margin":"16px " + ($('#kinds-eat').width() - $('#logo').width()+16)/2 +"px 0px auto"});
        ...
        $(window).on('resize', function(){
            if(($(window).width()- $('#eat,#main,#main-finish').width())/2 > 245)
            {
                ...
                if($('*').is('#kinds-eat'))
                {
                    $('#logo,#minihat-word').css({"margin":"5px " + ($('#kinds-eat').width() - $('#logo').width()+16)/2 +"px 0px auto"});
                    $('#hat div:nth-child(5)').css({"margin":"0px auto 0px "+($('#order').width() + 16 - $('.work-time').width())/2+"px"});
                }
            }
        });
        //при нажатии на "заказать обед" страница прокручиается вверх
        $("#miniHat a:nth-child(2)").click(function(){
